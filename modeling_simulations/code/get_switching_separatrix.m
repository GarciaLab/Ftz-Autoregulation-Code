...
mval_tol = 1e3; % how close a trajectory must be to the high s.s. to be
                % considered a successful switch (random search); L1 norm
...
for ii_a0 = 1:Na0
    ...
    for ii_r0 = 1:numel(r0_set)
        ...
        for ii_bis = 1:Nmax
            ...
            [~,ix] = min(abs(M(end) - ssR) + abs(P(end) - ssP));
            if ix == 3
    ...
    for ii = 1:Ntot
        ...
        for jj = 1:size(nin,1)
            if ~isnan(nin(jj,2)) && ~isnan(nin(jj,1))
                ...
                % check if switch succeeded
                [mval,ix] = min(abs(M(end) - ssR) + abs(P(end) - ssP));
                if mval < mval_tol  % otherwise inconclusive simulation
                    if ix == 3
                        Mmax = [Mmax;nin(jj,:)];
...
%% ----- SAVE RESULT ----- %
save('switching_separatrix_a0-r0-p0','Mmax_pool','Mmin_pool','fi');